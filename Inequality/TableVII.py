...
def reorder_povm_to_bell(effects, tol=1e-5):
    """
    Takes a raw list of POVM effects and pairs them into settings.

    Input:
        effects (list of np.array): The unordered quantum effects.

    Output:
        ordered_effects (list): List of effects ordered as [P1_a, P1_b, P2_a, P2_b, ...]
                                where P1_a + P1_b = Identity.
    """
    n_effects = len(effects)
    d = effects[0].shape[0]
    identity = np.eye(d, dtype=effects[0].dtype)

    used = np.zeros(n_effects, dtype=bool)
    ordered_effects = []
    ...
    for i in range(n_effects):
        ...
        # Search for a partner E2 such that E1 + E2 proportional to Identity
        for j in range(i + 1, n_effects):
            if used[j]: continue

            E2 = effects[j]
            total = E1 + E2

            # Check if total is scalar * Identity
            # We estimate the scalar by taking the mean of the diagonal
            scalar = np.trace(total) / d

            # Check deviation from scalar * I
            dist = np.linalg.norm(total - scalar * identity)
            ...
            if dist < tol:
                ...
                # Rescale so they sum EXACTLY to Identity (removing the 2/N factor)
                # If E1 + E2 = scalar * I, we need (E1+E2)/scalar = I
                scale_factor = 1.0 / np.real(scalar)

                ordered_effects.append(E1 * scale_factor)
                ordered_effects.append(E2 * scale_factor)

                found_match = True
                break

        if not found_match:
            # This happens if you have an odd number of effects or a hemisphere mesh
            pass

    return np.array(ordered_effects)
...
def Dvertices(effects, tol=1e-12):
    ...
    return P
def Bellvertices(num_settings):
    """
    Generates local vertices for 'num_settings' binary measurements.

    Input:
        num_settings (int): Number of measurement choices (N).

    Output:
        vertices (np.array): Shape (2^N, 2*N)

    Format:
        Each row is a deterministic strategy.
        Columns are organized as pairs: [Set1_+, Set1_-, Set2_+, Set2_-, ...]
        Values are 1/N (normalized) or 0.
    """
    N = num_settings
    # 1. Total deterministic strategies = 2^N
    num_vertices = 1 << N
    # 2. Total probabilities tracked = 2 outcomes * N settings
    num_columns = 2 * N
    # 3. Create container
    vertices = np.zeros((num_vertices, num_columns))
    # Normalization factor (so the total vector sums to 1)
    # This matches your script's logic where sum(Effects) = Identity
    val = 1.0 / N
    # 4. Fill matrix using bitwise logic
    for i in range(num_vertices):
        # 'i' is the strategy index (e.g., 5 is binary 101 -> outcomes +, -, +)
        for setting in range(N):
            # Check bit at position 'setting' to decide outcome (0 or 1)
            outcome = (i >> setting) & 1

            # Map to column index:
            # Setting 0 -> cols 0,1
            # Setting 1 -> cols 2,3
            col_idx = (2 * setting) + outcome
            vertices[i, col_idx] = val
    return vertices
...
def compute_eta_primal(A_verts, B_verts, N, M):
    """
...
if __name__ == "__main__":
    ...
    try:
        import Bases
        #N, M = Bases.octahedron_povm(), Bases.cube_povm()
        N, M = Bases.geodesic_povm(1), Bases.goldberg_povm(6)

        N=reorder_povm_to_bell(N)
        print("Computing Eta via Dual...")
        A_verts = Bellvertices(int(len(N)/2))
        B_verts = Dvertices(M)

        eta = compute_eta_primal(A_verts, B_verts, N, M)

        print(f"Critical Visibility (eta): {eta:.6f}")
